     ...
     let checkUserExist = await cartModel.findOne(
      ...
      for( i = 0 ; i < checkCartExist.items.length ; i++ ) {
        ...
        if( checkCartExist.items.productId != productId ){
          ...
          let totalPrice = checkCartExist.totalPrice + ( quantity * checkProductExist.price )
          
            let updateCartItems = await cartModel.findOneAndUpdate({ _id : cartId } , { $set : { totalPrice : totalPrice } , $push : { items : items } , $inc : { totalItems : 1 }} , { new : true })

            return res.status(201).send({ status : true , message : "Success" , data : updateCartItems })
        }
      }
...
const getCart = async function ( req , res ) {
  try { 
      ...
      if ( getCartData.items.length == 0) {
          return res.status(400).send({ status : false , message: "Items details not found or it may be deleted." })
      };
...
const updateCart = async function ( req , res ) {
  try{
    ...
    if( !checkUserId ) {
      return res.status(404).send({ status : false , message : "This userID does not exsit in your database , check your userId." })
    }
    ...
    if( Object.keys( body ).length == 0 ) {
      return res.status(400).send({ status : false , message : "You can not update empty body,please provide required credentials."})
    }
    ...
    if( !checkCartId ) {
      return res.status(404).send({ status : false , message : "This cartID does not exsit in your database , check your cartId." })
    }
    ...
    if ( checkCartId.items.length == 0 ) {
      return res.status(400).send({ status : false , message : "There are no items left in this cart or it might be deleted." })
  }
    ...
    if( !checkProductId ) {
      return res.status(404).send({ status : false , message : "This productID does not exsit in your database , it might be deleted." })
    }
     ...
     for ( i = 0 ; i < checkCartId.items.length ; i++ ) {
      ...
      if( checkCartId.items[i].productId == productId ) {
        ...
        if( removeProduct == 1 && checkCartId.items[i].quantity > 1 ) {
          ...
          checkCartId.totalPrice = ( checkCartId.totalPrice - ( checkCartId.items[i].quantity * checkProductId.price ))

           checkCartId.save()
           
           return res.status(200).send({ status : true , message : " Success " , data : checkCartId})
        } 
        ...
        if( removeProduct == 1 && checkCartId.items[i].quantity == 1 ) {
          ...
          let totalPrice = ( checkCartId.totalPrice - ( checkCartId.items[i].quantity * checkProductId.price ))


          let updatedCart = await cartModel.findOneAndUpdate({ _id : cartId } , { $pull : { items : newItems} , totalPrice : totalPrice , $inc : { totalItems : -1 } } , { new : true } )

          return res.status(200).send({ status : true , message : true , data : updatedCart })
        }
...
const deleteCart = async function ( req , res ) {
  try{ 
      ...
      let cartDeleted = await cartModel.findOneAndUpdate({ userId : userId } , { $set : { items : [] , totalPrice : 0 , totalItem : 0 }} , { new : true })

      if( !cartDeleted ) {
        return res.status(404).send({ status : false , message : "Cart does not exit in with this userId." })
      }
  
      return res.status(204).send({ status : true , message : "Cart succesully deleted."})
